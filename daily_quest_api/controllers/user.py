...
from daily_quest_api.entities import UserResponse
...
@user_routes.get('/users/', response_model=List[UserResponse])
def get_users(db: Session = Depends(get_db)):
    return user_service.get_all(db)

@user_routes.get('/users/{id}', response_model=UserResponse)
def get_user_by_id(id: str, db: Session = Depends(get_db)):
    return user_service.get_by_id(id, db)

@user_routes.post('/users', response_model=UserResponse)
